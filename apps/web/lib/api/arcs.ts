...
export interface Arc {
  ...
  seedsToResolveInArc: string[];
  coveredTurningPoints: number[];
...
export async function listArcs(
  ...
  const r = await fetch(`${BASE}/api/stories/${storyId}/sagas/${sagaId}/arcs`, {
    cache: "no-store",
  });
...
export async function getArc(storyId: string, arcId: string): Promise<Arc> {
  const r = await fetch(`${BASE}/api/stories/${storyId}/arcs/${arcId}`, {
    cache: "no-store",
  });
...
export async function planArcs(
  storyId: string,
  sagaId: string,
  currentState: string,
) {
  const r = await fetch(
    `${BASE}/api/stories/${storyId}/sagas/${sagaId}/arcs/plan`,
    {
      method: "POST",
