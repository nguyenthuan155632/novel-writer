...
export async function cancelBatch(storyId: string, batchId: string) {
  const r = await fetch(
    `${BASE}/api/stories/${storyId}/batches/${batchId}/cancel`,
    { method: "POST" },
  );
...
export async function retryBatch(storyId: string, batchId: string) {
  const r = await fetch(
    `${BASE}/api/stories/${storyId}/batches/${batchId}/retry`,
    { method: "POST" },
  );
  if (!r.ok) throw new Error(`retryBatch ${r.status}`);
  return r.json();
}
