...
const UpdateBibleSchema = z.object({
  ...
  realmLadder: z.array(z.string()).optional().nullable(),
  ...
  styleFewShots: z
    .array(
      z.object({ excerpt: z.string(), sourceChapter: z.number().optional() }),
    )
    .optional(),
});
...
const plugin: FastifyPluginCallback = (app, _opts, done) => {
  app.post<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      const [current] = await db
        .select({ version: storyBibles.version })
        .from(storyBibles)
      ...
      const [row] = await db
        ...
        .values({
          ...
          realmLadder: bible.realm_ladder ?? null,
    ...
    async (req, reply) => {
      ...
      const [next] = await db
        ...
        .values({
          ...
          powerSystem: patch.powerSystem ?? current.powerSystem,
          powerSystemKind: patch.powerSystemKind ?? current.powerSystemKind,
          cultivationSystem:
            patch.cultivationSystem !== undefined
              ? patch.cultivationSystem
              : current.cultivationSystem,
          bloodlineSystem:
            patch.bloodlineSystem !== undefined
              ? patch.bloodlineSystem
              : current.bloodlineSystem,
          realmLadder:
            patch.realmLadder !== undefined
              ? patch.realmLadder
              : current.realmLadder,
  ...
  app.put<{ Params: { storyId: string } }>(
    "/api/stories/:storyId/bible/style-few-shots",
    async (req, reply) => {
      const db = getDb();
      ...
      try {
        ...
        return reply
          .status(400)
          .send({ error: "validation_failed", details: (e as Error).message });
      }
      ...
      await db.insert(storyBibles).values({
        ...
        realmLadder: current.realmLadder,
