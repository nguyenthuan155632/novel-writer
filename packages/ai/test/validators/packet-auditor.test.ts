...
describe('auditPacket', () => {
  it('passes valid packet', () => {
    const r = auditPacket({ packet: basePacket as any, characters: [aliveChar], forbiddenRules: '', duePlantedSeeds: [] }, { genreFamily: 'cultivation' });
    expect(r.pass).toBe(true);
    expect(r.issues).toHaveLength(0);
  });

  it('flags dead character', () => {
    const r = auditPacket({ packet: basePacket as any, characters: [{ ...aliveChar, status: 'dead' }], forbiddenRules: '', duePlantedSeeds: [] }, { genreFamily: 'cultivation' });
    expect(r.pass).toBe(false);
    expect(r.issues[0]!.code).toBe('dead_character');
  });

  it('flags missing cliffhanger', () => {
    const r = auditPacket({ packet: { ...basePacket, cliffhanger: '' } as any, characters: [aliveChar], forbiddenRules: '', duePlantedSeeds: [] }, { genreFamily: 'cultivation' });
    expect(r.pass).toBe(false);
...
describe('auditPacket realm-jump gating', () => {
  it('fires realm_jump_excess when family=cultivation', () => {
    const result = auditPacket(
      { packet: baseRealmPacket, characters: [{ name: 'Lý Phong', status: 'alive', currentRealm: 'luyện khí' }], forbiddenRules: '', duePlantedSeeds: [] },
      { genreFamily: 'cultivation' },
    );
