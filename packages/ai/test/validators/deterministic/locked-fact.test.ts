...
function makeInput(content: string, lockedFacts: { topic: string; fact: string }[] = []): CheckInput {
  return {
    content,
    context: {
...
describe('lockedFactCheck', () => {
  it('passes when locked facts are respected', () => {
    const result = lockedFactCheck.run(makeInput(
      'Thanh kiếm Thần Long là bảo vật truyền thừa.',
  ...
  it('fails when topic mentioned without locked fact', () => {
    const result = lockedFactCheck.run(makeInput(
      'Thanh kiếm Thần Long là một vật phẩm bình thường.',
      [{ topic: 'Thanh kiếm Thần Long', fact: 'bảo vật truyền thừa' }],
    ));
  ...
  it('passes when locked fact topic is not mentioned', () => {
    const result = lockedFactCheck.run(makeInput(
      'Lam Trach luyện kiếm trong rừng.',
      [{ topic: 'Thanh kiếm Thần Long', fact: 'bảo vật truyền thừa' }],
    ));
    expect(result.pass).toBe(true);
  });
