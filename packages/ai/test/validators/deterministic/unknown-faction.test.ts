...
function makeInput(content: string, knownFactions: string[] = [], knownCharacters: string[] = []): CheckInput {
  return {
    content,
    context: {
...
describe('unknownFactionCheck', () => {
  it('passes when the faction-prefixed name is in knownFactions', () => {
    const result = unknownFactionCheck.run(
      makeInput('Tông môn Thiên Kiếm Môn họp khẩn vào nửa đêm.', ['Thiên Kiếm Môn']),
    );
  ...
  it('flags an unknown faction introduced after a faction prefix', () => {
    const result = unknownFactionCheck.run(
      makeInput('Tông môn Hỏa Vân Tông tuyên chiến với Bắc Di.', ['Thiên Kiếm Môn']),
    );
