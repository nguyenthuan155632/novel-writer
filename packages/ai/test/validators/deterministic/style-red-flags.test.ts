...
describe("styleRedFlagsCheck", () => {
  it("passes for clean content", () => {
    const result = styleRedFlagsCheck.run(
      makeInput("Lam Trach luyện kiếm một cách kiên trì."),
    );
  ...
  it("flags English red-flag patterns", () => {
    const result = styleRedFlagsCheck.run(
      makeInput("He leveled up and the system notified him."),
    );
  ...
  it("flags Vietnamese red-flag patterns", () => {
    const result = styleRedFlagsCheck.run(
      makeInput("Hệ thống thông báo: đột phá thành công."),
    );
    expect(result.pass).toBe(false);
  });
});
