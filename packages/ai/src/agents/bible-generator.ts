...
import { z } from 'zod';
import { parseCompletionJsonObject, withCompletionRetryRaw } from '../parse-completion-json.ts';
...
function responseSchemaForInput(input: GenerateBibleParams['input']) {
  ...
  return {
    ...bibleV2JsonSchema,
    required: [...new Set([...(bibleV2JsonSchema.required ?? []), 'cultivation_system'])],
  };
}
...
export async function generateBible(params: GenerateBibleParams): Promise<GenerateBibleResult> {
  ...
  for (let attempt = 0; attempt < BIBLE_VALIDATION_ATTEMPTS; attempt++) {
    ...
    if (res.finishReason === 'length') {
      ...
      throw new Error('bible_generator: completion hit max output tokens before producing a complete bible');
    }
  ...
  throw lastValidationError ?? new Error('bible_generator: failed to produce a valid bible');
}
