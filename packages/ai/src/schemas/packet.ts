...
export const ChapterPacketSchema = z.object({
  ...
  cliffhanger: z.string().max(PACKET_LIMITS.cliffhanger).optional(),
  chapterPurpose: z.string().min(1).max(PACKET_LIMITS.chapterPurpose).default("plot_progression"),
  endingMode: z.string().min(1).max(PACKET_LIMITS.endingMode).default("quiet_transition"),
...
export const CHAPTER_PACKET_JSON_SCHEMA: JsonSchema = {
  ...
  required: ['chapterNumber', 'goal', 'requiredEvents', 'charactersPresent', 'conflict', 'forbiddenMoves'],
  properties: {
